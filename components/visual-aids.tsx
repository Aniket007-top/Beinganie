    ...
    <div className="rounded-[1.25rem] border border-border-subtle bg-surface-secondary p-4 dark:border-slate-800 dark:bg-slate-900/70">
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        {loop.map((item) => (
          <div
            key={item}
            className="rounded-[1.25rem] border border-google-yellow/20 bg-google-yellow/10 px-4 py-5 text-center font-mono text-xs uppercase tracking-[0.16em] text-google-yellow"
          >
            {item}
          </div>
        ))}
